        ...
        <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                                            ...
                                            <div class="text-sm text-green-700">
                                                ...
                                                <ul class="list-disc list-inside">
                                                    @foreach ($equipo->evaluaciones as $eval)
                                                        <li>{{ $eval->juez->name }} -
                                                            {{ number_format($eval->calificacion_total, 2) }} pts</li>
                                                    @endforeach
                                                </ul>
                                            </div>
                                        @endif
                ...
                <div class="flex justify-between items-start">
                        ...
                        @if ($equipo->proyecto && ($esMiembro || auth()->user()->tieneRol('juez') || auth()->user()->tieneRol('admin')))
                            <div class="mt-4 flex flex-wrap gap-3">
                                @if ($equipo->proyecto->link_repositorio)
                                    <a href="{{ $equipo->proyecto->link_repositorio }}" target="_blank"
                                ...
                                @if ($equipo->proyecto->link_demo)
                                    <a href="{{ $equipo->proyecto->link_demo }}" target="_blank"
                                ...
                                @if ($equipo->proyecto->link_presentacion)
                                    <a href="{{ $equipo->proyecto->link_presentacion }}" target="_blank"
                                        ...
                                        <svg class="w-5 h-5" fill="currentColor" viewBox="0 0 20 20">
                                            <path fill-rule="evenodd"
                                                d="M3 4a1 1 0 011-1h12a1 1 0 011 1v2a1 1 0 01-1 1H4a1 1 0 01-1-1V4zm0 4a1 1 0 011-1h6a1 1 0 011 1v6a1 1 0 01-1 1H4a1 1 0 01-1-1V8zm11-1a1 1 0 00-1 1v6a1 1 0 001 1h2a1 1 0 001-1V8a1 1 0 00-1-1h-2z"
                                                clip-rule="evenodd" />
                                        </svg>
                                        Ver Presentación
                                    </a>
                    ...
                    <div class="flex gap-2">
                        @if ($esMiembro && !$esLider)
                            @if (!$fueEvaluado)
                                <!-- Abandonar equipo (solo si NO fue evaluado) -->
            ...
            @if ($equipo->proyecto && $esMiembro)
                ...
                <div
                    ...
                    <div class="flex items-start justify-between mb-4">
                        <div class="flex-1">
                            <div class="flex items-center gap-3 mb-2">
                                ...
                                <!-- Botón Editar Proyecto (Solo Líder y si puede editar) -->
                                @if ($esLider && $puedeEditar)
                    ...
                    <div class="bg-gray-50 rounded-lg p-4 mb-4">
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
                            ...
                            @php
                                ...
                                $tareasOk =
                                    $totalTareas >= $proyecto->evento->min_tareas_proyecto &&
                                    $totalTareas > 0 &&
                    ...
                    @elseif(
                        $proyecto->cumpleRequisitosMinimos() &&
                            in_array($proyecto->estado, ['borrador', 'pendiente_revision', 'en_progreso']))
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    @if ($equipo->proyecto)
                        <div class="bg-white rounded-xl shadow-sm p-6">
                            ...
                            <!-- Contenedor de tareas (siempre presente) -->
                                        ...
                                        <div data-tarea-id="{{ $tarea->id }}"
                                            ...
                                            <div class="flex items-start justify-between">
                                                <div class="flex items-start gap-3 flex-1">
                                                    ...
                                                    @if ($puedeMarcar)
                                                        <form method="POST"
                                                            action="{{ route('equipos.tareas.toggle', [$equipo, $tarea]) }}">
                                                            @csrf
                                                ...
                                                @if ($esLider)
                                                    <div class="flex gap-2 ml-4">
                                                        ...
                                                        <form method="POST"
                                                            action="{{ route('equipos.tareas.destroy', [$equipo, $tarea]) }}"
                ...
                <div class="space-y-6">
                    ...
                    @if ($equipo->proyecto)
                        <div class="bg-white rounded-xl shadow-sm p-4">
                            ...
                            @php
                                ...
                                $tareasCompletadas = $equipo->proyecto
                                    ->tareas()
                    ...
                    @if ($esMiembro)
                        <div class="bg-white rounded-xl shadow-sm">
                            ...
                            <div id="mensajesContainer" class="h-64 overflow-y-auto p-4 space-y-3" id="chat-messages">
                                @php
                                    $mensajes = $equipo
                                        ->mensajes()
                                    ...
                                    @foreach ($pendientes as $solicitante)
                                        <div class="p-3 bg-yellow-50 rounded-lg border border-yellow-100" data-solicitud-id="{{ $solicitante->id }}">
                                            ...
                                            <div class="flex gap-2 mt-2">
                                                <form method="POST"
                                                    action="{{ route('equipos.aceptar-miembro', [$equipo, $solicitante->id]) }}"
                                                    class="flex-1">
                                                    @csrf
                                                    <button type="submit"
                                                        class="w-full px-3 py-1.5 bg-green-600 text-white rounded text-xs font-medium hover:bg-green-700">
                                                        Aceptar
                                                    </button>
                                                </form>
                                                <form method="POST"
                                                    action="{{ route('equipos.rechazar-miembro', [$equipo, $solicitante->id]) }}"
                                                    class="flex-1">
                                                    @csrf
                                                    <button type="submit"
                                                        class="w-full px-3 py-1.5 bg-red-600 text-white rounded text-xs font-medium hover:bg-red-700">
                                                        Rechazar
                                                    </button>
                                                </form>
                                            </div>
